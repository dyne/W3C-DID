...
generate-sandbox-did: prepare-generation ## Create a new sandbox DID document on did.dyne.org
	./restroom-test -s ${RR_SCHEMA} -h ${RR_HOST} -p ${RR_PORT} -u v1/sandbox/pubkeys-accept.chain -a /tmp/pubkeys-request.json | jq .
generate-sandbox-did-local: prepare-generation ## Create a new sandbox DID document on localhost
